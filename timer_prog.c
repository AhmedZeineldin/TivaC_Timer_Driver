...
u32 GPT_Time_Capture (u8 copy_u8TimerNumber) //after 268 seconds overflow occours
{
    ...
    return( ( GPTMTAR(copy_u8TimerNumber) * SECONDS_TO_US)  / CLOCK_FREQUENCY ); //CAPTURES THE VALUE OF THE LAST EVENT OCCOURS in us
}
...
void GPT_DiffrenceBetweenTwoEvents(u8 copy_u8TimerNumber,u32 * buffer) //when counter value is 1 take the value
{
    ...
    else
    {
        ...
        temp_data[counter] = GPT_Time_Capture (copy_u8TimerNumber) ;
        if (counter % 2 != 0 )
        {
            if (temp_data[1] > temp_data[0]) //no overflow happened
            {
                *(diffrence[copy_u8TimerNumber]) =  temp_data[1] - temp_data[0] ;
            }
            else //case overflow happened
            {
                *(diffrence[copy_u8TimerNumber]) = (  0xFFFFFFFF - temp_data[0] ) + ( temp_data[1] - 0x00000000 ) ;
            }
...
void delay_func_us (u8 timer_num, u32 delay)
{
    ...
    while( (( GPTMTAR(timer_num)- snap_shot) < (delay * 16) ) )
    {
...
//    {
        GPT_DiffrenceBetweenTwoEvents(Timer0 , diffrence[0]);
